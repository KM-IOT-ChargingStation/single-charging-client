...
public class ScanUnLockStrategy implements RequestStrategy {
    ...
    public void process(RequestBody requestBody, ResponseBody responseBody, ChannelHandlerContext ctx) {
        ...
        ScanUnlockResponseDto requestDto =
                JSONObject.
                        parseObject(requestBody.getData(), ScanUnlockResponseDto.class);
...
//        long bikeId = getBikeId(requestDto.getKid(), dockStartId);
        ...
        ScanUnLockRequestDto response =
                new ScanUnLockRequestDto(siteId,
                        dockId, bikeId,
                        requestDto.getUid(),
                        gbs,
                        HardWareUtils.getInstance().
                                getUtcTimeStampOnDevice(timezone));//这是东8区时间戳
        ...
        responseBody.setFunctionCodeArray(ClientFunctionCodeType.ScanUnLock);
